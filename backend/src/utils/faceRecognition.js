...
// Inicializar cliente de AWS si hay credenciales
if (process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY && process.env.AWS_REGION) {
  ...
  console.log('✅ AWS Rekognition inicializado para verificación facial');
} else {
  console.log('⚠️ AWS Rekognition no configurado. Usando simulación.');
}

/**
 * Compara la foto del INE con la selfie para verificar que sea la misma persona
 */
async function compararCaras(imagenINE, imagenSelfie) {
  if (!rekognitionClient) {
    console.log('🔄 Usando simulación de verificación facial');
    return { match: true, similarity: 85, mensaje: '✅ Verificación facial simulada exitosa' };
  }
